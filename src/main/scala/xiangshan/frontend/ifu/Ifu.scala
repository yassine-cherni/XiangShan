...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  private val instrDelimiter  = Module(new InstrBoundary)
  ...
  private val s0_fetchBlock = genFetchBlockInfo(s0_ftqFetch)
  /* *****************************************************************************
   * IFU Stage 1
  ...
  private val s2_iCacheAllRespWire = fromICache.valid && isICacheMatch(fromICache.bits, s2_fetchBlock)
  ...
  private val s2_icacheMeta = WireDefault(0.U.asTypeOf(Vec(FetchPorts, new ICacheMeta)))
  s2_icacheMeta(0) := genICacheMeta(fromICache.bits)
  ...
  instrDelimiter.io.req.cacheData := Cat(s2_rawData, s2_rawData) >> Cat(s2_fetchBlock(0).startVAddr(5, 0), 0.U(3.W))
...
// used for s3_prevLastHalfData calculation.
  ...
  private val twoFetchIdentifiedCfi = VecInit.tabulate(FetchBlockInstNum)(i =>
    ...
    if (i == 0) s2_fetchBlock(0).identifiedCfi(0)
    else Mux(rawIsRvc(i), s2_fetchBlock(0).identifiedCfi(i), s2_fetchBlock(0).identifiedCfi(i - 1))
  )
  ...
  for (i <- 0 until IBufferInPortNum / 2) {
    ...
    val highIdx    = s3_alignCompactInfo.instrIndex(i + IBufferInPortNum / 2).value
  ...
  frontendTrigger.io.data   := 0.U.asTypeOf(Vec(IBufferInPortNum + 1, UInt(16.W))) // s4_alignInstrData
  frontendTrigger.io.frontendTrigger := io.frontendTrigger
  private val s4_alignTriggered = frontendTrigger.io.triggered

  /* ** send to IBuffer ** */
  private val ignoreRange = Cat(Fill(IBufferInPortNum - IfuAlignWidth, 1.U(1.W)), ~s4_ignore) // maybe remove
  ...
  uncacheFlushWb.bits.pd     := uncachePd
  uncacheFlushWb.bits.pc     := s4_uncachePc
  uncacheFlushWb.bits.ftqIdx := s4_fetchBlock(0).ftqIdx
  uncacheFlushWb.bits.takenCfiOffset := s4_fetchBlock(0).takenCfiOffset.bits
  uncacheFlushWb.bits.misEndOffset := uncacheMisEndOffset
  uncacheFlushWb.bits.cfiEndOffset := DontCare
  uncacheFlushWb.bits.jalTarget := DontCare
  ...
  uncacheFlushWb.bits.instrRange := s4_uncacheRange
  ...
  when(s4_reqIsUncache) {
    ...
    uncacheFlushWb.bits.pd(s4_shiftNum).valid  := true.B
    uncacheFlushWb.bits.pd(s4_shiftNum).isRVC  := uncacheIsRvc
    uncacheFlushWb.bits.pd(s4_shiftNum).brType := brType
    uncacheFlushWb.bits.pd(s4_shiftNum).isCall := isCall
    uncacheFlushWb.bits.pd(s4_shiftNum).isRet  := isRet
  }
  ...
  toFtq.pdWb(0) := Mux(wbValid, checkFlushWb(0), uncacheFlushWb)
  ...
  private val s3_icachePerfInfo = RegEnable(io.fromICache.perf, s2_fire)
  ...
  generatePerfEvent()
  perfAnalyzer.io.ifuPerfCtrl.ftqReqValid     := fromFtq.req.valid
  perfAnalyzer.io.ifuPerfCtrl.ftqReqReady     := fromFtq.req.ready
  perfAnalyzer.io.ifuPerfCtrl.backendRedirect := backendRedirect
  perfAnalyzer.io.ifuPerfCtrl.ifuWbRedirect   := wbRedirect.valid
  perfAnalyzer.io.ifuPerfCtrl.ifuS0Flush(0)   := s0_flushFromBpu(0)
  perfAnalyzer.io.ifuPerfCtrl.ifuS0Flush(1)   := s0_flushFromBpu(1)
  perfAnalyzer.io.ifuPerfCtrl.ifuS3Valid      := s3_valid
  perfAnalyzer.io.ifuPerfCtrl.ifuS2Fire       := s2_fire
  perfAnalyzer.io.ifuPerfCtrl.ifuS3Fire       := s3_fire
  perfAnalyzer.io.ifuPerfCtrl.icacheRespValid := icacheRespAllValid
  ...
  perfAnalyzer.io.perfInfo.checkPerfInfo.misPred(0)       := checkFlushWb(0).bits.misEndOffset.valid
  perfAnalyzer.io.perfInfo.checkPerfInfo.misPred(1)       := checkFlushWb(1).bits.misEndOffset.valid
  perfAnalyzer.io.perfInfo.checkPerfInfo.misEndOffset(0)  := checkFlushWb(0).bits.misEndOffset.bits
  perfAnalyzer.io.perfInfo.checkPerfInfo.misEndOffset(1)  := checkFlushWb(1).bits.misEndOffset.bits

  perfAnalyzer.io.perfInfo.toIBufferInfo.ibufferValid  := io.toIBuffer.valid
  perfAnalyzer.io.perfInfo.toIBufferInfo.ibufferReady  := io.toIBuffer.ready
